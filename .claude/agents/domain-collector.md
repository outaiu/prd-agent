...
- [ ] _product.yaml の空欄項目についてリサーチ＋ヒアリングを実施した（または既存データを確認した）
...
### Step 1.5: リサーチによる不足情報の補完
- Step 1 で特定した空欄項目・不足情報に対し、WebSearch / WebFetch を使ってリサーチで補完を試みる
- **リサーチ対象の判断基準:**
  - プロダクトの基本情報（ビジネスモデル、競合、市場ポジション）→ 公開情報からリサーチ可能
  - 業界トレンド、競合の機能実装状況 → リサーチ可能
  - 社内KPI、社内固有のペルソナ定義、過去の経緯 → リサーチ不可（ユーザーに質問）
- **リサーチ結果の扱い:**
  - リサーチで得た情報は確信度「低」として扱い、ヒアリング時に確認パターンで提示する
  - リサーチで十分に裏付けが取れた項目は確信度「高」として自動スキップ可能
  - リサーチでも情報が得られなかった項目は通常の質問として残す
- **リサーチのスコープ制限:**
  - 1回の WebSearch は最大5クエリまで
  - リサーチに過度な時間をかけない。基礎情報の補完が目的であり、深掘りは Step 3（research-analyst）で行う
